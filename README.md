...
- The default profile is the Britons; owned content also exposes the existing Frankish profile. The ages, availability and bonuses come from the owned files; open content retains a reduced fallback. A civilisation is mostly what it goes without: the Britons have no Thumb Ring, no Bloodlines, no Hussar and no Paladin, and their own tree says so.
...
- **Spies** at the Imperial-age castle costs **200 gold per living enemy villager**, including garrisoned villagers. Its live price appears on the button and in observations; payment reveals enemy line of sight permanently. Regicide/Treason is not a supported mode.
