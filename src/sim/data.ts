...
export interface UnitRules {
   ...
   * militia and the champion are both the barracks' 1.
   */
  trainButton?: number;
...
export interface TechRules {
  ...
  name: string;
  /**
   * Its cell in the command grid, 1-15 across then down, from the DAT's
   * `research_locations[*].button_id`. A chain shares its cell -- Forging,
   * Iron Casting and Blast Furnace are all 1 -- so nothing moves when one
   * lands.
   */
  button?: number;
...
export const FALLBACK_RULES: GameRules = {
  ...
  units: {
    ...
    // The Imperial tier, generated from the DAT rather than transcribed. Like
    // every other upgrade target, none of these is trainable until its own
    ...
    arbalester: {
      ...
      range: 5.0, projectileSpeed: 7, launchHeight: 1.5,
    },
    cavalier: {
      age: 3,
    ...
    'heavy-cavalry-archer': {
      ...
      range: 4.0, projectileSpeed: 7, launchHeight: 1.5,
    },
    'elite-longbowman': {
      age: 3,
      ...
      range: 6.0, projectileSpeed: 7, launchHeight: 1.5,
    },
    onager: {
      age: 3,
  ...
  technologies: {
    ...
    'feudal-age': {
      techId: 101, name: 'Feudal Age', cost: cost(500), researchSeconds: 130, button: 11,
      researchedAt: 'town-center', requiresAge: 0, grantsAge: 1, effects: [],
    },
    'castle-age': {
      techId: 102, name: 'Castle Age', cost: cost(800, 0, 200), researchSeconds: 160, button: 11,
...
export function rulesFromManifest(manifest: ContentManifest): GameRules {
  ...
  return {
    ...
    units: {
      ...
      champion: unit('champion', 'barracks'),
...
function technologies(
  ...
  for (const [key, tech] of Object.entries(manifest.technologies ?? {})) {
    ...
    result[key] = {
      ...
      name: tech.name,
      help: tech.text?.help,
      button: tech.button ?? fallback?.button,
