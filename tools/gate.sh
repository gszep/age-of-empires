...
# The sentinel tools/hooks/guard_commit.sh checks: written only on GREEN, and
# removed first so a failed run cannot leave yesterday's pass behind.
rm -f .local/gate.ok
...
git rev-parse HEAD > .local/gate.ok 2>/dev/null || date > .local/gate.ok
echo "GATE GREEN"
