...
- The 2026-08-28 playtest round: buildings carry the DAT's armour and attack upgrades reach what they hit, an attacker follows what it is attacking, one bounded rule for what a worker turns to next, the mill's technologies through effect command type 1, farm re-sowing as an option that starts off, the villager build menu laid out from the DAT's own button slots, the trebuchet packed and unpacked, the wonder (without a victory condition), a fifteen-deep refundable training queue, a selected group shown as its members with double-click-to-take-the-kind, and fog that forgets a sheep once it is yours.
...
- Per-tick cost over a full 900-second match (seed 102): re-measured on
  2026-08-29 on the generated map, warm, with both example AIs playing and
  **1308 live entities**: median **0.95ms**, p90 1.09ms, p99 **1.49ms**, worst
  **6.07ms**, against a 50ms budget at 20Hz. The old figures for comparison
  (before the shot model, the technology tree, five-minute corpses and a board
  with five times the trees): median 0.58ms, p99 1.72ms, worst 11.7ms. The
  median rose with the entity count; the worst tick — the number that matters —
  halved. Batch throughput on the fuller board is ~150-370x depending on
  co-load, down from 431x, in proportion to the entities simulated.
- Forests are solid and clean: grown with the original's cost-ordered
  round-robin growth and closed by its `cleanTerrain` two-pass (pinholes and
  diagonal squeezes filled), asserted by `mapgen.test.ts` rather than sampled
  by hand. Crossing one still means going round it.
- The board is generated by the original's own two primitives
  (`src/sim/mapgen.ts`, from the owned scripts in the resources depot —
  `depot_813784/.../gamedata_x2`, where `Arabia.rms` and `land_resources.inc`
  live): distance bands are per-axis **boxes**, not rings, so resources reach
  band corners; gold, stone and berries flood-fill as tight lumps; the neutral
  wood covers the script's own ~8% of the board and fades off the start areas
  probabilistically; thirty lone stragglers scatter over open ground. Both
  halves stay exact mirrors — a named divergence from the original's
  independent draws, kept because the paired evaluation batch rests on it.
  `GameState.terrain` now carries a per-tile DAT terrain id (grass/forest);
  the renderer still draws one ground, because the blend-mask mapping is
  still missing (see the terrain note below).
